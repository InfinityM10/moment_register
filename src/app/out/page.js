...
import { PunchOut } from '../actions/moment'
...
function PunchOutContent() {
  ...
  const [message, setMessage] = useState('Processing punch out...')
  ...
  const handlePunchOutWithLocation = async () => {
    try {
      setStatus('loading')
      ...
      navigator.geolocation.getCurrentPosition(
        async (position) => {
          ...
          const result = await PunchOut(fingerPrint, location, parsedUserData)
  ...
  const handlePunchOutWithoutLocation = async () => {
    try {
      setStatus('loading')
        ...
        {status === 'prompt' && (
          <div className="flex flex-col items-center space-y-4">
            ...
            <p className="text-sm text-gray-600 text-center">
              We use your location to verify your punch out. This helps ensure accurate attendance tracking.
            </p>
            <button
            ...
            <button
              onClick={handlePunchOutWithoutLocation}
